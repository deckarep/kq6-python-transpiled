...
class rm280(KQ6Room):
	...
	def init():
		...
		if ((global12 == 145) and proc913_0(41)):
			...
			(global0 posn: 51 121)
		#endif
		(global0 init: reset: loop: 9 cel: 0 setScale: Scaler 105 90 139 121)
		(shopOwner init:)
		(mints init:)
...
class takeMintScr(Script):
	...
	def changeState(param1 = None):
		...
		match state = param1
			...
			#end:case
			case 2:
				(global0 reset: 0 posn: (mints approachX:) (mints approachY:))
				cycles = 2
			#end:case
			case 3:
...
class mints(View):
	...
	def doVerb(param1 = None):
		...
		match param1
			case 5:
				if (global153 < 3):
					if (not (global0 has: 23)):
						(global2 setScript: takeMintScr)
					else:
						(global91 say: noun param1 35)
					#endif
				else:
					(global91 say: 7 5 32)
				#endif
			...
			case 1:
				if (global153 < 3):
					(global91 say: noun param1 33)
				else:
					(global91 say: noun param1 32)
				#endif
			#end:case
			else:
				(super doVerb: param1 &rest)
...
class firstWallLookScr(Script):
	...
	def changeState(param1 = None):
		...
		match state = param1
			...
			#end:case
			case 1:
				(global91 say: register 1 50 0 self)
			#end:case
			case 2:
				(global1 handsOn:)
				(self dispose:)
...
class northShelf(Feature):
	...
	def doVerb(param1 = None):
		...
		match param1
			case 1:
				if ((local1 + 1) == 0):
					(global2 setScript: firstWallLookScr 0 noun)
					(local1 += 2)
				else:
					(global91
						say:
...
class eastShelf(Feature):
	...
	def onMe(param1 = None):
		...
		return ((super onMe: param1 &rest) and ((param1 x:) > 119))
	#end:method
